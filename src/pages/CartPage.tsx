...
const CartPage: React.FC = () => {
  ...
  const handleDecrease = (id: number, size: string | null, name?: string) => {
    ...
    if (item && item.quantity <= 1) {
      if (confirm(`Số lượng sản phẩm đang là 1. Bạn có muốn xóa "${name ?? 'sản phẩm'}" khỏi giỏ hàng?`)) {
        removeFromCart(id, size);
        toast.info(`Đã xóa ${name ?? 'sản phẩm'} khỏi giỏ hàng`, { autoClose: 1500 });
      }
      return;
  ...
  React.useEffect(() => {
    try {
      cart.forEach((item) => {
        ...
        if (!el) return;
        if (document.activeElement === el) return; // don't overwrite focused input
      ...
      <div className="lg:col-span-8">
        <div className="bg-white shadow-lg rounded-2xl p-6 border border-gray-100">
          <div className="flex justify-between items-center mb-4">
            ...
            {cart.length > 0 && (
              <button onClick={handleClearCart} className="text-red-500 text-sm font-medium hover:text-red-600 transition">Xóa tất cả</button>
            )}
          </div>
          ...
          {cart.length === 0 ? (
            <div className="text-center py-10">
              ...
              <Link to="/" className="inline-block px-5 py-2 bg-gradient-to-r from-blue-500 to-indigo-500 text-white rounded-md hover:opacity-90 transition">Tiếp tục mua sắm</Link>
            </div>
          ) : (
              ...
              {cart.map((item) => (
                <div key={`${item.id}-${item.size ?? 'default'}`} className="flex items-center gap-5 py-4 rounded-lg transition">
                  <div className="w-24 h-24 flex-shrink-0">
                    <img src={item.image} alt={item.name} className="h-full w-full object-cover rounded-lg border border-gray-200 shadow-sm" />
                  </div>
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-sm text-gray-500 mt-1">{(item.price * item.quantity).toLocaleString()}.000đ</p>
                      </div>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <div className="flex items-center border rounded overflow-hidden bg-gray-50">
                        <button onClick={() => handleDecrease(item.id, item.size ?? null, item.name)} className="w-8 h-8 flex items-center justify-center text-lg hover:bg-gray-100">-</button>
                        <input
                          type="number"
                          ...
                          onBlur={(e) => {
                            ...
                            if (v === '') {
                              el.value = el.dataset.prev ?? String(item.quantity);
                              return;
                            }
                            ...
                            const q = Math.max(1, Math.floor(parsed));
                        ...
                        />
                        <button onClick={() => handleIncrease(item.id, item.size ?? null)} className="w-8 h-8 flex items-center justify-center text-lg hover:bg-gray-100">+</button>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div className="bg-white shadow-lg rounded-2xl p-6 border border-gray-100 lg:sticky lg:top-20">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium text-gray-800">{cart.length > 0 ? shippingFee.toLocaleString() : '0'}.000đ</span>
            </div>
            <hr />
